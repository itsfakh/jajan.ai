...
# PAGE CONFIG & CSS
# ==================================
st.set_page_config(page_title="ScanStruk AI V3", page_icon="🧾", layout="wide")
...
try:
    ...
    # ⚠️ PENTING: Paste kembali link URL Google Sheets Keuanganmu di bawah ini!
...
# ==================================
# SIDEBAR: AKSES PENGGUNA & BUDGETING
# ==================================
...
# INOVASI OPSi 2: Pengaturan batas anggaran bulanan rapi di sidebar
target_anggaran = st.sidebar.number_input("Batas Anggaran Bulanan (Rp)", min_value=0, value=2000000, step=50000)
...
total_hari_ini = 0
total_bulan_ini = 0
baris_terakhir_user = None # INOVASI OPSI 3: Wadah penampung baris untuk fitur Undo
...
try:
    ...
    for index, row in enumerate(semua_data):
        ...
        if len(row) >= 5 and row[4] == nama_pengguna:
            ...
            # Kunci posisi nomor baris transaksi terakhir milik user aktif
...
# ==================================
st.markdown('<div class="hero"><h1>🧾 ScanStruk AI V3</h1><p style="font-size:18px;">Hybrid Budgeting System & Smart Account Security</p></div>', unsafe_allow_html=True)

# INOVASI OPSI 2: Tampilan Sisa Anggaran Bulanan bergaya Progress Bar Dinamis
...
# Tampilan Ringkasan Kecil Harian dan Grafik Distribusi
        ...
        if image_file:
            with st.spinner("🤖 AI sedang membaca struktur teks dan nominal..."):
                ...
                try:
                    ...
                    # AMAN ANTI-PATAH: Struktur pemisah baris pendek untuk menangkal SyntaxError string literal
...
if len(data_user_semua) > 0:
    ...
    # INOVASI OPSI 3: Tombol merah pembatal transaksi terakhir (Undo System)
    with col_hist2:
        if baris_terakhir_user:
            st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
            if st.button("🗑️ Hapus Transaksi Terakhir (Undo)", type="primary"):
                try:
                    sheet.delete_rows(baris_terakhir_user)
